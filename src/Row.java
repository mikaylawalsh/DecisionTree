...
public class Row {
     ...
     *
     * From the "attributeValues" hashmap, we get all keys (equivalent to column
     * names in the data table) as Strings, and the method returns them as a Set
     * of Strings.
     *
     ...
     *
     * This method gets the corresponding value of THIS row to the passed-in
     * "attributeName", which is one of the column names.
     * Remember, this class represents a SINGLE row, so we are able to look up
     * one specific value for the passed-in attributeName.
     *
     ...
     *
     * We set the passed-in value as the value for attributeName in this current
     * row.
     *
     ...
     * For example, if attributeAccesses were [color, color, color, highFiber,
     * lowCarb, lowCarb], this method would output [color, highFiber, lowCarb].
     * There may be duplicates in the attributeAccesses list because in their
     * Node class, students may have a for loop comparing the datum value of an
     * attribute with each of the edge values, so the attribute value may be
